...
local pipeline = Effects.pipe({
	...
	function(character, add, remove)
		local function update()
			...
			if superball and superball:IsA("Tool") and head and head:IsA("BasePart") then
				add(character)
			else
				remove(character)
			end
...
local function superballTrajectoryVisualizer(root)
	...
	local function onChanged(new, old)
		if
			old == nil
			or getLocalSetting(new, "practiceWeaponDisplay") ~= getLocalSetting(old, "practiceWeaponDisplay")
		then
			if undo then
				undo()
			end
