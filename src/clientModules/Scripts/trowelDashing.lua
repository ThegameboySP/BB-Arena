...
local projFilter = Effects.pipe({
	...
	Effects.children,
	function(proj, add)
		if not proj:IsA("BasePart") then return end
		if not proj.Name:find("Bomb") then return end
...
local function getContigousBricks(brick, dir, bricks)
	...
	local nextSide = workspace:Raycast(
		brick.Position + dir * (brick.Size.X/2 - 0.01),
		dir * 0.2
	)
...
local function getSize(trowel, playerPos, trowelCF)
	...
	for _, child in pairs(trowel:GetChildren()) do
		...
		local bottom = workspace:Raycast(
			child.Position + Vector3.new(0, -child.Size.Y/2 + 0.01, 0),
			Vector3.new(0, -0.8, 0)
		)
		...
		if 
			top and isBrick(top.Instance)
			and bottom and not isBrick(bottom.Instance)
		then 
