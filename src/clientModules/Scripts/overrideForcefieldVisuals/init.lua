...
local function overrideForcefieldVisuals(root)
	...
	local function hook(add, remove, step)
		...
		local function onDescendantAdded(descendant)
			...
			if
				descendant.Parent
				and descendant.Parent:FindFirstChild("Humanoid")
		...
		table.insert(
			connections,
			CollectionService:GetInstanceRemovedSignal("VisualForcefieldNode"):Connect(function(instance)
					...
					if data.removing then
						if data.character:IsDescendantOf(game) then
							...
							for _, child in data.character:GetChildren() do
								if
									child:IsA("ForceField") and not CollectionService:HasTag(child, "ForceFieldVisual")
								then
									newForcefield = child
									break
								end
