...
local function trowelVisualizer(root)
    ...
    local Targeting = require(LocalPlayer.PlayerScripts
        :WaitForChild("ToolObjects")
        :WaitForChild("Core")
        :WaitForChild("Targeting"))
    ...
    RunService.Heartbeat:Connect(function()
        ...
        local vectorConstructor = Vector3.new(
            math.ceil(worldPos.X-0.5),
            math.floor(worldPos.Y*100)*0.01,
            math.ceil(worldPos.Z-0.5)
        )
