...
local function blendIn(msg)
	...
	t1.Completed:Once(function()
		local t2 =
			TweenService:Create(Frame, TweenInfo.new(0.75, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), FULL)
		t2:Play()
...
local function blendOut()
	local t1 = TweenService:Create(Frame, TweenInfo.new(0.75, Enum.EasingStyle.Quad, Enum.EasingDirection.In), SMALL)
	local t2 =
		TweenService:Create(Objective, TweenInfo.new(0.75, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), TEXT_OFF)
	t2:Play()
...
return function()
	local function update()
		if LocalPlayer:GetAttribute("Objective") then
			blendIn(LocalPlayer:GetAttribute("Objective"))
		else
			blendOut()
		end
	end

	LocalPlayer:GetAttributeChangedSignal("Objective"):Connect(update)
	update()
end
