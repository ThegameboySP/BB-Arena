...
local function settingEntry(props, hooks)
	...
	local descriptionBounds =
		TextService:GetTextSize(setting.description .. "\n", DESCRIPTION_TEXT_SIZE, Enum.Font.Gotham, descriptionSize)
	...
	elseif setting.type == "keybind" then
		control = e(button, {
			...
			position = UDim2.new(1, -10, 0.5, 0),
			anchor = Vector2.new(1, 0.5),
			text = if keybindCallback and keybindCallback.name == setting.name
	...
	return e("TextButton", {
		...
		[Roact.Event.MouseButton1Click] = if not setting.valid
			then nil
			else function()
