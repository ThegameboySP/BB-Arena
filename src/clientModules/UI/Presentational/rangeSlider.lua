...
local function percentSlider(props, hooks)
	...
	return e(
		...
		{
			Range = e(props.inactive and "TextLabel" or "TextBox", {
				...
				RichText = true,
				Text = string.format(
					[[<font size="18">%d%s —</font> %d%s <font size="18">— %d%s</font>]],
					props.min,
					sign,
					props.value,
					sign,
				...
				[Roact.Event.FocusLost] = if props.inactive
					then nil
						...
						if enterPressed then
							...
							textboxRef:getValue().Text = string.format(
								[[<font size="18">%d —</font> %d <font size="18">— %d</font>]],
								props.min,
								props.value,
								props.max
							)
