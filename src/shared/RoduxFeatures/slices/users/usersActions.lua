...
function actions.setAdmin(userId, admin, byUser)
	return function(store)
		...
		if byUser == nil or (getAdmin(state, byUser) > admin and getAdmin(state, userId) < getAdmin(state, byUser)) then
			store:dispatch({
				type = "users_setAdmin",
				payload = {
					userId = userId,
...
function actions.setReferee(userId, isReferee, byUser)
	return function(store)
		...
		if byUser == nil or (getAdmin(state, byUser) >= GameEnum.AdminTiers.Owner) then
			store:dispatch({
	...
	return function(store)
		...
		store:dispatch(
			actions.saveSettings(
				...
				state.users.locallyEditedSettings
			)
		)
